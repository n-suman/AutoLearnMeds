...
# Add the path to the system linker config + always run ldconfig — the
# ld.so.cache doesn't survive runtime restarts even if the conf file does.
...
echo ""
echo "IMPORTANT — to keep the Colab runtime alive across long runs, paste this"
echo "into a NEW notebook cell and Run (it'll print one line per minute, forever):"
echo ""
echo "  import time, datetime"
echo "  while True:"
echo "      print(f'[keepalive] {datetime.datetime.utcnow().isoformat()}', flush=True)"
echo "      time.sleep(60)"
echo ""
echo "(Each flushed print triggers a websocket message kernel->browser, defeating"
echo " both the kernel-idle timer AND the browser-session-idle detector. Stop"
echo " with Runtime > Interrupt execution. The notebooks/00_bootstrap.ipynb file"
echo " also has this as its last cell for convenience.)"
echo "============================================================"
